...
func (d *digest) checkSum() []byte {
	...
	var currentLength = (((d.len / BlockSize) << 7) + uint64(dataLen)) << 3

	d.padBuf[0] = 0x01 | byte(d.passes<<3)
	d.padBuf[1] = byte(d.olen << 3)

	putu32(d.padBuf[2:], uint32(currentLength))
	putu32(d.padBuf[6:], uint32(currentLength>>32))

	var endLen = uint32((dataLen + 138) & ^127)
...
func (d *digest) write160() (out []byte) {
	...
	putu32(out[0:], s0+mix1600(s5, s6, s7))
	putu32(out[4:], s1+mix1601(s5, s6, s7))
	putu32(out[8:], s2+mix1602(s5, s6, s7))
	putu32(out[12:], s3+mix1603(s5, s6, s7))
	putu32(out[16:], s4+mix1604(s5, s6, s7))
...
func (d *digest) write192() (out []byte) {
	...
	putu32(out[0:], s0+mix1920(s6, s7))
	putu32(out[4:], s1+mix1921(s6, s7))
	putu32(out[8:], s2+mix1922(s6, s7))
	putu32(out[12:], s3+mix1923(s6, s7))
	putu32(out[16:], s4+mix1924(s6, s7))
	putu32(out[20:], s5+mix1925(s6, s7))
